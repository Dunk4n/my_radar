...
((my_getfloat(line[3]) - my_getfloat(line[1]) != 0) ? (my_getfloat(line[3]) -
my_getfloat(line[1])) : 1);
    trans->vit.x = (my_getfloat(line[5]) / sqrt(1 + pow(a, 2))) *
((my_getfloat(line[3]) < my_getfloat(line[1])) ? -1 : 1);
    trans->vit.y = trans->vit.x * a;
    trans->alp = atan2(my_getfloat(line[4]) - my_getfloat(line[2]),
my_getfloat(line[3]) - my_getfloat(line[1])) * 180 / M_PI + 90;
